...
export default function UpcomingRegistrationCard({ race }) {
  ...
  return (
    <div className="registration-card-vertical">
      {/* Image section - top third */}
      <Link
        href={`/race/${encodeURIComponent(getRaceId(race))}`}
        className="registration-card-image-link"
      >
        <div className="registration-card-image">
          ...
          >
            <defs>
              <linearGradient id={`grad-${getRaceId(race)}`} x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" style={{ stopColor: '#0f5c5b', stopOpacity: 1 }} />
                <stop offset="100%" style={{ stopColor: '#1a8785', stopOpacity: 1 }} />
              </linearGradient>
            </defs>
            <rect width="400" height="200" fill={`url(#grad-${getRaceId(race)})`} />
            <path
              d="M50 150 Q75 120, 100 140 T 150 130 T 200 145 T 250 135 T 300 150 T 350 140"
              stroke="rgba(255,255,255,0.3)"
              strokeWidth="3"
              fill="none"
            />
            <circle cx="80" cy="90" r="25" fill="rgba(255,255,255,0.2)" />
            <path
              d="M80 70 L85 90 L75 90 Z M75 90 L80 110 L70 95 Z M85 90 L80 110 L90 95 Z"
      ...
      {/* Title section */}
      <div className="registration-card-content" onClick={toggleExpanded}>
        ...
        <div className="registration-card-distance-badges">
          <span className="registration-card-distance">
        ...
        {/* Icons row */}
        <div className="registration-card-icons">
          {race.medal && (
        ...
        <div className="registration-card-info">
          <p className="registration-card-race-date">
            <strong>Race Date:</strong> {formatDate(race.date)}
          </p>
          
          {race.earlyBirdCost && (
            <p className="registration-card-price">
              <strong>Early Bird:</strong>{" "}
              <span className="race-card-registration-cost">{race.earlyBirdCost}</span>
              <br />
              <small>
                {formatDate(race.registrationStart, "registration") || "N/A"} → {formatDate(race.earlyBirdDeadline, "registration") || "N/A"}
              </small>
            </p>
          )}
        ...
        {expandedFields.length > 0 && (
          <button
            className="registration-card-expand-btn"
            onClick={(e) => {
              e.stopPropagation();
              toggleExpanded();
            }}
          ...
          >
            ...
            <svg
              ...
              height="16"
              viewBox="0 0 24 24"
              className={`registration-card-chevron ${expanded ? 'expanded' : ''}`}
            >
        ...
        {expanded && (
          <div className="registration-card-expanded">
