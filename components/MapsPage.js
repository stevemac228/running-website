		...
		async function init() {
			...
			for (let i = 0; i < results.length; i++) {
				...
				if (startLatLng) {
					...
					// Create GPX layer with custom start icon and hidden end icon
					try {
						lightLayer = new L.GPX(blobUrl, {
							async: true,
							polyline_options: { color: color, weight: 0, opacity: 0 }, // Hide polylines
							marker_options: {
								startIconUrl: iconUrl,
								endIconUrl: "", // Hide end marker
								iconSize: [18, 30],
								iconAnchor: [9, 30],
								clickable: true
							},
														...
														if (!grp.layer) {
															try {
																const layer = new L.GPX(grp.blobUrl || gpxUrl, {
																	...
																	marker_options: { 
																		startIconUrl: buildSmallPin(grp.color),
																		endIconUrl: "",
																		iconSize: [18, 30],
																		iconAnchor: [9, 30],
																		clickable: true
			...
			if (!group.layer) {
				try {
					...
					const layer = new ref.L.GPX(group.blobUrl, {
						...
						marker_options: { 
							startIconUrl: buildSmallPin(group.color),
							endIconUrl: "",
							iconSize: [18, 30],
							iconAnchor: [9, 30],
							clickable: true
