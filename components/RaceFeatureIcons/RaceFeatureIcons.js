const ICON_DEFINITIONS = [
  ...
  {
    flag: "shirt",
    tooltip: "This race has a tshirt!",
    render: (className) => (
      <svg
        className={className}
        width="18"
  ...
  {
    flag: "reception",
    tooltip: "This race has a reception!",
    render: (className) => (
      <svg
        className={className}
        width="18"
...
/**
 * Renders the medal, shirt, and reception icons with consistent tooltip styling.
 */
export default function RaceFeatureIcons({
  medal,
  shirt,
  reception,
  iconClassName = "racecard-icon",
}) {
  const flags = { medal, shirt, reception };

  return ICON_DEFINITIONS.filter(({ flag }) => flags[flag]).map(
    ({ flag, tooltip, render }) => (
      <div key={flag} className="tooltip-container">
